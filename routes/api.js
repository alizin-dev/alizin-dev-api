...
loghandler = {
    ...
    noturl: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de url'
    },
    notgcname: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro grupo nome'
        },
    notgcicon: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros do gcicon'
        },
    notpp: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro pp'
        },
    notbg: {
        status: false,
        creator: `${creator}`,
    ...
    notmemberCount: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro memberCount'
        },
    notquery: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro query'
        },
    notkata: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro kata'
    },
    nottext: {
        status: false,
        creator: `${creator}`,
    ...
    notnabi: {
        ...
        creator: `${creator}`,
        code: 406, 
        message: 'insira o parâmetro nabi'
    },
    nottext3: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro text3'
    },
    nottheme: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro theme'
    },
    notusername: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro username'
    },
    notvalue: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro value'
    },
    invalidKey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidlink: {
        status: false,
        creator: `${creator}`,
        message: 'erro, talvez seu link não seja válido.'
    },
    invalidkata: {
        status: false,
        creator: `${creator}`,
...
router.post("/apikey", async (req, res, next) => {
  ...
  if(listkey.includes(key)) {
    res.json({
      message: 'apikey já está registrado'
    });
  } else {
    listkey.push(key);
    res.json({
      message: `${key} registrado com sucesso no banco de dados apikey`
    });
  }
});
...
router.delete("/apikey", async(req, res, next) => {
	...
	if(listkey.includes(key)) {
		res.json({
			message: 'apikey não estava lá antes'
			})
			} else {
	listkey.splice(key, 1)
	res.json({
		message: 'apikey removido com sucesso' 
});
 }
});
...
router.get('/stalk/npm', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!query) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro query"})
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (!Apikey) return res.json(loghandler.notparam)
		if (listkey.includes(Apikey)){
		if (!type) return res.json({status: false, creator, code: 404, message: 'insira o parâmetro type, type yang tersedia : base64 , base32'})
...
router.get('/info/cuaca', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(!kota) return res.json({status: false, code: 406, message: 'insira o parâmetro kota'})
  if(listkey.includes(apikey)) {
    Cuaca(kota)
    .then((data) => {
      res.json(data)
...
router.get('/muslim/hadits', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!kitab) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro kitab"})
    if (!nomor) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro nomor"})
...
router.get('/muslim/quran', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!surah) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro surah"})
    if (!ayat) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro ayat"})
...
router.get('/muslim/jadwalshalat', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!kota) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro kota"})
...
router.get('/fun/simisimi-ar2', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!text) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro text"})
...
router.get('/info/wikipedia', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!search) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro search"})
...
router.get('/fakedata', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!country) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro country"})
...
router.get('/hilih', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!kata) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro kata"})
...
router.get('/music/chordlagu', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!lagu) return res.json({ status : false, creator : `${creator}`, message : "insira o parâmetro kata"})
